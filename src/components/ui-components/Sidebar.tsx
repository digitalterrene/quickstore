      ...
      {showSidebar && (
        <div className=" ">
          {!isExcluded && (
            <div
              id="docs-sidebar"
              className="hs-overlay w-72 bg-gray-50 h-screen lg:sticky  overflow-y-auto [--auto-close:lg] hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform hidden fixed top-0 start-0 bottom-0 z-[60]      md:relative   border-e border-gray-200  pt-4 pb-10   lg:block lg:translate-x-0 lg:end-auto lg:bottom-0 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500 dark:bg-neutral-800 dark:border-neutral-700"
                        ...
                        {revised_industries?.map(
                          ({ industry, categories }: any) => (
                            ...
                            >
                              <button
                                type="button"
                                className="hs-accordion-toggle border-b  w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-800   hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:text-neutral-200"
                                aria-expanded="true"
                                aria-controls={`accordion-${industry}-child`}
                              >
                                <a
                                  href={`/industries/${industry}`}
                                  className="flex gap-3   w-full hover:text-blue-600 items-center w-fit text-md capitalize"
                                >
                                  <img
