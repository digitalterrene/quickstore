...
import { useAccountInformationFWCInformationInputs } from "@/context/account-information";
...
export default function FWCInformationTab({
  fwc_information,
  ...
  const { inputs, setInputs } = useAccountInformationFWCInformationInputs();
  const [gender, setGender] = useState<string>("male"); // Default to 'male'
  const [formData, setFormData] = useState<AccountInformationFWCObject>({
    ...inputs,
    ...fwc_information,
  });
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const key_to_update = "fwc_information";
    const value_to_update = { ...formData, gender };
    const id = toast.loading("Updating fwc information...");
    ...
    if (res.response === "ok") {
      toast.update(id, {
        render: "FWC information updated successfully",
        type: "success",
        isLoading: false,
      });
    } else {
      toast.update(id, {
        render: "Failed to update fwc information",
        type: "error",
        isLoading: false,
      });
    }
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-gray-600">
              This information is required by Rodlip, and is considered public
              domain information for regulation purposes. However just like all
              information governed by Rodlip, it is insured
            </p>
          </div>
